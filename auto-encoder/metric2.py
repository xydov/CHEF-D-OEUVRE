...
for i in range(n):
    ...
    ssim_noisy = get_ssim(x_test[i].reshape(28, 28), x_test_noisy[i].reshape(28, 28))
    psnr_noisy = get_psnr(x_test[i].reshape(28, 28), x_test_noisy[i].reshape(28, 28))
    
    # Calculate Metrics for Denoised Image
    ssim_denoised = get_ssim(x_test[i].reshape(28, 28), decoded_imgs[i].reshape(28, 28))
    psnr_denoised = get_psnr(x_test[i].reshape(28, 28), decoded_imgs[i].reshape(28, 28))

    # Display noisy image with metrics
    ax = plt.subplot(3, n, i + 1)
    plt.imshow(x_test_noisy[i].reshape(28, 28), cmap="gray")
    plt.title(f"Degraded Image\nSSIM: {ssim_noisy:.2f}\nPSNR: {psnr_noisy:.2f}")
    plt.axis("off")

    # Display denoised image with metrics
